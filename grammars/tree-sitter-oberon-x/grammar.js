  ...
  rules: {
    ...
    module: $ => choice(
      seq(
        ...
        // Oberon-X dialect extension (docs/plan.md M5): "DO" accepted as a synonym for
        // "BEGIN" everywhere the latter introduces a statement_seq, not a breaking rename —
        // base Oberon-2 corpus files stay valid Oberon-X input.
        optional(seq(
          choice($.kBegin, $.kDo),
    ...
    procedure_body: $ => seq(
      ...
      // Oberon-X dialect extension (docs/plan.md M5): "DO" as a synonym for "BEGIN" here too.
      optional(seq(choice($.kBegin, $.kDo), optional($.statement_seq))),
      $.kEnd
    ),
    ...
    // Oberon-X dialect extension (docs/plan.md M5, not in normative EBNF): a negated-condition
    // counterpart to while/if, "UNLESS" expression "DO" statement_seq "END" — runs the body
    // when expression is false, no ELSE/ELSIF (mirrors while_statement's shape minus the loop).
    unless_statement: $ => seq(
      $.kUnless, $.expression, $.kDo, optional($.statement_seq), $.kEnd
    ),

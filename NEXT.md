...
Working tree: `grammars/tree-sitter-oberon2/`. Run `tree-sitter generate -o gen-src &&
tree-sitter test` from that directory after every change. `gen-src/` (the real generated
`parser.c`, `grammar.json`, `node-types.json`, `tree_sitter/`) is gitignored; `src/` holds
`scanner.c` (real, committed) plus symlinks (`parser.c`, `grammar.json`, `node-types.json`,
`tree_sitter/`, also committed) pointing into `gen-src/` — `tree-sitter test`/`tree-sitter parse`
hardcode reading `src/parser.c` with no override flag, so the symlinks are what let those
commands keep working unmodified while the actual generated bytes live outside `src/`. The
symlinks are set up once and don't need recreating after a plain regenerate — only run the
`ln -s` commands again if a symlink is ever accidentally deleted.
...
- `src/` holds `scanner.c` (real, tracked) plus symlinks into `gen-src/` for the generated files
  (also tracked, so a fresh checkout has the symlinks even before the first `generate` run —
  they'll just dangle until `tree-sitter generate -o gen-src` populates their targets).
  `gen-src/` itself is gitignored. See "What to do" above for the exact command.

...
## M4.2 — CI: fail on undiffed report change ✅ (round 33, 2026-08-11)

**Scoping question resolved first, per `NEXT.md`.** `corpus/roots.toml`'s four roots are
machine-local absolute paths (license reasons, see that file's header) — no CI runner has them,
so `xoft corpus run` against the real corpus can't run in CI at all. Asked the user; chose
vendoring a small fixture corpus into the repo, specifically: `voc` from its public GitHub
(`github.com/vishaps/voc`, GPL-3.0) and `oberon-a` from its Aminet archive (freely
redistributable per that package's own readme: "Distribution: Freeware, under the GNU General
Public Licence"). `amiga-oberon-31` (commercial, A+L AG) and `stj` (no public source URL at
hand) are left out — user's own call, "when in doubt, leave it out" — so CI covers two of the
four roots; the full four-root run stays a local-only check against `corpus/roots.toml`.

**No `xoft-core`/`xoft-cli` code changes were needed.** `corpus run`'s `--roots`/`--allowlist`/
`--out` flags (already present since M4.1, all three default-but-overridable) are exactly the
seam a second, CI-scoped corpus needs — pointing them at a second `roots.toml` was the whole
mechanism. This is purely a data-vendoring + workflow-file change.

`corpus/fixtures/`, new: `roots.toml` (two `[[root]]` entries, `voc`/`oberon-a`, repo-relative
paths), `allowlist.toml` (empty — every vendored file is picked to already be clean), `NOTICE.md`
(provenance + license text per root, and why the other two roots are excluded), `report.json`
(checked in, generated by an actual `corpus run` against these fixtures). Six files per root,
picked small (429–6431 bytes) and directory-diverse (5 `voc` subdirs, 4 `oberon-a` subdirs),
explicitly avoiding every path already in `corpus/allowlist.toml` so the fixture set stays
100%/100% by construction — confirmed by running `corpus run` against it before committing
anything, not assumed.

`oberon-a`'s fetch needed two extraction layers: Aminet's `Obrn-A_1.6_src.lha` unpacks to three
*more* `.lha` archives (`source4.lha`, `source5.lha`, `texts2.lha`) rather than source files
directly — `lha x -w=<dir>` (Homebrew's `lha`, already installed) on each, in sequence, before
any `.mod` files were visible to pick from.

`.github/workflows/ci.yml`, new (first CI config of any kind in this repo): `cargo test
--workspace`, then `corpus run --roots corpus/fixtures/roots.toml --allowlist
corpus/fixtures/allowlist.toml --out corpus/fixtures/report.json`, then `git diff --exit-code
corpus/fixtures/report.json` — fails the build if regenerating the fixture report produces a
diff from what's checked in, which is what "undiffed report change" means at fixture scale.
Verified the whole chain locally before writing the workflow (`cargo test --workspace` green;
`corpus run` on the fixtures twice back-to-back produced byte-identical `report.json` both
times, confirming the reproducibility the CI check depends on) — not run through actual GitHub
Actions (no push in this round; ask the user how they want to verify it if that matters before
relying on it).

**M4 is done.** Next milestone is M5 (toy dialect Oberon-X), tagged **Opus** in `docs/plan.md`
line 147 — flag to the user before starting rather than silently continuing on whatever model
is running.
